...
ui <- fluidPage(
  ...
  numericInput('price.num', label = 'Price (in dollars)', min = 0, value = 0),

  # Include a `numericInput()` widget with the label "Quantity"
  # It should have a default value of 1 and a minimum value of 1
  numericInput('price.quantity', label = 'Quantity', value = 1, min = 1),
...
server <- function(input, output) {
  ...
  output$cost <- renderText({
    cost <- input$price.num * input$price.quantity
    return(cost)
  })
